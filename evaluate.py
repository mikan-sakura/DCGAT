...
parser.add_argument('--corpus_type', type=str, default="CMeIE_V2_pos", help="NYT, WebNLG, NYT*, WebNLG*")
...
def evaluate(model, data_iterator, params, ex_params, mark='Val'):
    ...
    for batch in tqdm(data_iterator, unit='Batch', ascii=True):
        ...
        input_ids, attention_mask, triples, input_tokens, pos_ids, bio_tags = batch
        bs, seq_len = input_ids.size()

        # inference
        with torch.no_grad():
            pred_seqs, pre_corres, xi, pred_rels = model(input_ids, attention_mask=attention_mask,pos_ids=pos_ids, token_ids=bio_tags,
                                                         ex_params=ex_params)

            # (sum(x_i), seq_len)
            pred_seqs = pred_seqs.detach().cpu().numpy()
            # (bs, seq_len, seq_len)
           #pre_corres = pre_corres.detach().cpu().numpy() if pre_corres is not None else None
